...
class config:
    def __init__(self):
        ...
        # logging set
        logging_path = self.root / f'loggers_{self.postFix}'
        ...
        self.decode_hidden_dims = [64, self.input_dim]
        ...
        self.checkpoint = self.root / f"weights_{self.postFix}"
        if not os.path.exists(self.checkpoint):
            os.makedirs(self.checkpoint, exist_ok=True)
        
        self.load_weights = True
        if self.load_weights:
            self.weight_path = self.root / f'weights_{self.postFix}/042_.pth.tar'
        ...
        self.valid_visualization = self.root / f"valid_visualize_{self.postFix}"
